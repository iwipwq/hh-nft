  ...
  : describe("randomIpfsNft", () => {
      ...
      describe("fulfillRandomWords", () => {
        it("requestRandomWords가 NFTRequested 이벤트를 발생시키면 동작", async () => {
          await new Promise(async (resolve, reject) => {
            ...
            const fulfillRandomWordsTxReceipt = await fulfillRandomWordsTx.wait(
              1
            );
            ...
            const blockNum =
              await vrfCoordinatorV2Mock.provider.getBlockNumber();
      ...
      describe("withdraw", () => {
        it("requestNft를 요청하면 mintFee만큼 계약에 돈이 들어오고 withdraw를 요청하면 계약에 들어있는 balance를 정상적으로 인출", async () => {
          ...
          console.log(
            "before",
            (await randomIpfsNft.getContractBalance()).toString()
          );
          const afterRequestNftBalance = (
            await randomIpfsNft.getContractBalance()
          )[0];
          assert.equal(
            initialBalance.add(mintFee).toString(),
            afterRequestNftBalance.toString()
          );
          ...
          const afterWithdrawBalance = (
            await randomIpfsNft.getContractBalance()
          )[0];
          assert.equal(
            afterRequestNftBalance.sub(mintFee).toString(),
            afterWithdrawBalance.toString()
          );
